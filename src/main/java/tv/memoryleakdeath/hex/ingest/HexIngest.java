...
public class HexIngest {
	...
	private static Options registerCommandLineOptions() {
		Option portOption = Option.builder().argName("[port]").desc("rtmp port (default is: " + DEFAULT_RTMP_PORT + ")")
				.hasArg(true)
				.numberOfArgs(1).option(RTMP_PORT_OPTION_ARG).optionalArg(false).required(false).valueSeparator('=')
				.type(Integer.class).build();
		Option maxConnectionsOption = Option.builder().argName("[max]")
				.desc("max client connections to allow (default is: " + DEFAULT_MAX_CONNECTIONS + ")")
		...
		Option maxWorkerThreadsOption = Option.builder().argName("[threads]")
				.desc("max worker threads to create (default is: " + DEFAULT_MAX_WORKER_THREADS + ")")
				.hasArg(true).numberOfArgs(1).option(MAX_WORKER_THREADS_ARG).optionalArg(false).required(false)
				.valueSeparator('=').type(Integer.class).build();
